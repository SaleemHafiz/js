...
(function () {
    ...
    function init(fn) {
        if (document.readyState === "complete" || document.readyState === "interactive") {
            ...
            window.addEventListener("DOMContentLoaded", () => setTimeout(fn, 1200));
        }
    }

    /******** LOAD LIBS (same as Violentmonkey version) ********/
    ...
    /******** DOWNLOAD FUNCTION (UNCHANGED UI LOGIC) ********/
    function triggerDownloads(data, teacherName, tableId) {
        ...
        const worksheet = XLSX.utils.aoa_to_sheet([
            ['Student','Date','Course','Juzz','Page','Lesson Detail','Additional','Section','Page','Add. Detail'],
            ...data
        ]);
        ...
        // HTML EXPORT (same as your original)
...
${document.getElementById(tableId).outerHTML}
        ...
        html2canvas(container, {
            ...
            useCORS: true,
            onclone: (doc) => {
                const el = doc.getElementById('dashboard-container');
                el.style.width = "2100px";
    ...
    async function buildMasterTable() {
            ...
            <table class="table table-sm table-striped table-hover mb-0" id="main-summary-table" style="width:100%; border-collapse: separate; border-spacing: 0;">
                <thead>
                    <tr>
                        ${['Student','Date','Course','Juzz','Page','Lesson Detail','Additional','Section','Page','Add. Detail']
                        .map((h,i)=>`
                        <th style="min-width:140px; padding:12px; background:#1e4db7; color:white; position:sticky; top:0; z-index:${i===0?40:20}; border-bottom:2px solid #000; ${i===0?'left:0;':''}">
                            ${h}
                        </th>`).join('')}
                    </tr>
                </thead>
        ...
        document.getElementById('master-history-body').innerHTML =
            results.map(r => `
            <tr style="font-size:13px; vertical-align:middle;">
                <td style="font-weight:700; color:#1e4db7; background:#f9f9ff; position:sticky; left:0; z-index:10; border-right:2px solid #1e4db7;">
                    ${r[0]}
                </td>
        ...
        document.getElementById('multi-download-btn').onclick =
            () => triggerDownloads(results, teacherName, 'main-summary-table');
    }

    init(buildMasterTable);
